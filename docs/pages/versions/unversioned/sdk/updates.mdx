...
There are various build-time configuration options that control the behavior of the library. For most apps, these configuration values should be set in the [app config](/workflow/configuration/) under the [`updates` property](/versions/latest/config/app/#updates).
...
Each time you build a binary for your app it includes the native code present at the time of the build as well as native configuration, and this unique combination is represented by a unique string called a runtime version. A remote update targets one runtime version, indicating that only binaries with a matching runtime version can load the remote update.
...
The `"nativeVersion"` policy is provided for projects with that wish to define their runtime compatibility based on the project's current `"version"` and `"buildNumber"` (iOS) or `"versionCode"` (Android) properties.
...
It's important to know that this policy does require management of the native version numbers manually between each build.
...
> **warning** The `fingerprint` runtime policy is still in beta as of SDK 51 and not yet considered stable. Use may result in unexpected behavior.
